package com.example.crud.configuration.crud.specification;

import com.example.crud.configuration.crud.specification.request.FilterRequest;
...
public class SearchSpecification<T> implements Specification<T> {
    ...
    public Predicate toPredicate(@NonNull Root<T> root, @NonNull CriteriaQuery<?> query, @NonNull CriteriaBuilder cb) {
        try {
            ...
            for (FilterRequestByColumn filterRequestByColumn : this.request.getFiltersByColumn()) {

                List<Path<?>> keys = new ArrayList<>();
                for (String key : filterRequestByColumn.getKey()){
                    Path<?> keyBuild = buildAbsolutePath(root, key);
                    keys.add(keyBuild);
                }
                predicate = filterRequestByColumn.getOperator().build(
                        root,
                        cb,
                        filterRequestByColumn,
                        predicate,
                        keys
                );
            }
            ...
            for (SortRequest sort : this.request.getSorts()) {
                ...
                orders.addAll(sort.getDirection().build(root, cb, sort, path));
            }

            query.orderBy(orders);
            log.debug("Applied filter: {} and filter by column {}, and sort: {}", this.request.getFilters(), this.request.getFiltersByColumn(), this.request.getSorts());
            return predicate;
        } catch (Exception e) {
            e.printStackTrace();
